    ...
    <div className="min-h-screen bg-gray-100 p-8 font-sans">
      <div className="max-w-7xl mx-auto bg-white p-6 rounded-xl shadow-lg">
        <div className="flex justify-between items-center mb-6 border-b pb-4">
          <h1 className="text-3xl font-extrabold text-gray-800">District Master</h1>
          <button
            onClick={handleAdd}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentItems.map((district, index) => {
                    ...
                    const state = states.find(s => s.iState_id === district.iState_id);
                    return (
                      <tr key={district.iDistric_id} className="hover:bg-blue-50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {displayIndex}
                        </td>
                        <td className="px-6 py-4 text-sm text-gray-900">
                          {district.cDistrict_name}
                        </td>
                        <td className="px-6 py-4 text-sm text-gray-900">
                          {state?.cState_name || `ID: ${district.iState_id}`}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                            district.bactive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                          }`}>
                            {district.bactive ? 'Active' : 'Inactive'}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                          ...
                          <button
                            onClick={() => handleDeactivateClick(district.iDistric_id)}
            ...
            {totalPages > 1 && (
              <div className="flex justify-between items-center mt-6">
                <span className="text-sm text-gray-700">
                  Showing <span className="font-semibold">{indexOfFirstItem + 1}</span> to <span className="font-semibold">{Math.min(indexOfLastItem, filteredDistricts.length)}</span> of <span className="font-semibold">{filteredDistricts.length}</span> results
                </span>
